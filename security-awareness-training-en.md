...
- Hacker (eg. ransomware) attacks on companies and organizations have increased significantly lately
...
- Email addresses (are often used to authentication eg. in company networks)
- etc.
...
- Especially recepionists need to be careful when receiving calls or welcoming a so called "printer repair tecnician" on the campus
- Examples:
...
- Verify the domain: only service@paypal.com is valid, customerservicepaypal@gmail.com not
  - Be aware that the second e-mail address doesn't get filtered out by mail servers and spam filters and therefore is interesting for attackers
...
- Minimum requirements for passwords:
  - Avoid Leetspeak (replacing of characters by similar looking symbols and numbers, eg. @ for "a" or 3 for "e")
...
- Avoid tokens that are sent via SMS to your phone, better use tokens that change every few seconds are are generated by an app
...
- Hackers can easily set up their own WiFi with the same name as the original one (e.g. Library- or train-WiFi)
  ...
  - Preferebly use cable connections instead of WiFi

---

# VPN

- VPNs don't make a connection "more secure" as today most connections are alredy encrypted using SSL/TLS
- VPN is useful 
  - if you want to hide from your internet provider which sites you visit
  - if you want to hide to a webpage from which country you are coming (and eg. want to activate content that would not be available without VPN)

---

- Commercial VPN providers promise to encrypt user data and therefore not being able to access and uncrypt it, although this is difficult to verify
...
- Computers: Altough you need to log in in order to use it data can be read from it if its disc is connected to another computer or an USB-Stick with a second operating system is connected to it
- Better encrypt them with tools like BitLocker, VeraCrypt on Windows, FileVault on Mac and LUKS on Linux
- The same applies to hard discs and USB-sticks: Once connected to a computer data can be read from them if they are not ecrypted
...
- Alternatively encrypt files locally with tools like Cryptomater and BoxCryptor before sending them eg. to Dropbox
...
- You can never be 100% "secure" from attacks, you can only manage and migitate against the risks that you are being faced with
